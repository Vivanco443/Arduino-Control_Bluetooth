...
void loop()
{
  ...
  if (get_char != CARACTER_INICIO_CMD) return; // si no hay indicaciÛn de inicio del sistema, volver  loop ().
  ...
  if (ard_command == LECTURA_ARDUDROID_CMD) { 
    ...
    Serial.println(analogRead(A0));  // Ejemplo: Leer y enviar valor analÛgico del Pin de Arduino
    return;  // Listoe. regrese al loop();
  }
}

// 2a) seleccionar el pin # solicitado para la acciÛn digitalWrite
void set_digitalwrite(int pin_num, int pin_value)
{
  ...
  case 13:
    ...
    // adicione su cÛdigo aquÌ, para este n˙mero de pin del Arduino
    break;
  case 12:
    pinMode(12, OUTPUT);
